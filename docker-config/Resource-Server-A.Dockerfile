...
WORKDIR /tmp/
RUN mvn -f pom.xml --projects resource-server-a install -DskipTests

FROM eclipse-temurin:24
COPY --from=maven_tool /tmp/resource-server-a/target/Study23-resource-server-a-1.0.0-SNAPSHOT.jar application.jar
